...
def search(pq,node,opn):
    # Search a node using recursion.
    if opn == pq[node]:
        return node
    if has_children(pq,node):
        for i in children(pq,node):
            # As pq is a min-heap it should search only if
            # the value of opn is bigger than the value of the current node
            if opn[0] >= pq[i][0]:
                res = search(pq,i,opn)
                if res is not None:
                    return res
...
def update(pq,opn,npn):
    # Update funtion exchanges opn to npn
    mh_position = search(pq,root(pq),opn)
    if mh_position is None:
        return 
    else:
        pq[mh_position] = npn
        i = mh_position
        # Annealing in case heap is disordered
        while i != root(pq) and get_data(pq, i) < get_data(pq, parent(i)):
            p = parent(i)
            exchange(pq, i, p)
            i = p 
...
#Initialize parameters using list comprehensions
